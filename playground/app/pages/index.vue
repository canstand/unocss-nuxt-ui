...
useHead({
  ...
  meta: [
    { name: 'description', content: 'Use UnoCSS with Nuxt UI without Tailwind CSS' },
  ],
})
...
<template>
  <div>
    <UHeader>
      <template #title>
        <svg
          width="1020" height="200" viewBox="0 0 1020 200" fill="none" xmlns="http://www.w3.org/2000/svg"
        ...
        <img src="https://unocss.dev/logo.svg" alt="UnoCSS Logo" class="ms-4 h-6 inline-block">
        <span class="translate-y-1">UnoCSS</span>
      </template>
    </UHeader>
    <UMain>
      <UContainer>
        <UPageHeader title="unocss-nuxt-ui playground" description="Use UnoCSS with Nuxt UI without Tailwind CSS" />
        <div class="flex flex-col items-center">
          <div class="mt-6xl flex gap-lg items-center justify-center">
            <UButton
              variant="soft" color="neutral" aria-label="Toggle dark mode"
              :icon="isDark ? 'i-lucide-sun' : 'i-lucide-moon'" @click="handleToggleDarkMode"
            />

            <UButton
              variant="soft" color="neutral" icon="i-lucide-github" as="a"
              href="https://github.com/canstand/unocss-nuxt-ui" target="_blank" rel="noopener"
              aria-label="unocss-nuxt-ui on GitHub"
            />
          </div>

          <div class="mt-6xl flex flex-wrap gap-lg items-center justify-center">
            <UButton v-for="color in colors" :key="color" :color="color" class="capitalize">
              {{ color }}
            </UButton>
          </div>

          <div class="mt-lg flex flex-wrap gap-lg items-center justify-center">
            <UButton v-for="variant in buttonVariants" :key="variant" :variant color="primary" class="capitalize">
              {{ variant }}
            </UButton>
            <UButton disabled>
              Disabled
          ...
          <div class="mt-6xl flex flex-wrap gap-lg max-w-200 items-center justify-center">
            <USelect
              v-for="variant in selectVariants" :key="variant" :items="items" :placeholder="variant"
              :variant="variant" class="w-48 capitalize" aria-label="Select a fruit or a vegetable"
            />
          </div>
