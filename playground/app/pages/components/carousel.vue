...
<template>
  ...
  <div class="min-h-0 space-y-11">
    <template v-if="classNames">
      <UCarousel v-slot="{ item }" v-bind="bind" :items="items" :ui="{ item: 'basis-[70%] transition-opacity ease-in-out [&:not(.is-snapped)]:opacity-10', container: 'h-[352px]' }" class="mx-auto max-w-xl w-full">
        <img :src="item.src" class="rounded-lg">
      </UCarousel>
    </template>
    <template v-else-if="autoHeight">
      <UCarousel v-slot="{ item }" v-bind="bind" :items="items" :ui="{ container: 'transition-[height] duration-200' }" class="mx-auto max-w-md w-full">
        <img :src="item.src" class="rounded-lg">
      </UCarousel>
    </template>
    <template v-else>
      <UCarousel v-slot="{ item }" v-bind="bind" :items="items" class="mx-auto w-[320px]" :ui="{ container: 'h-[336px]' }">
        <img :src="item.src" class="rounded-lg">
      </UCarousel>

      <template v-if="orientation === 'horizontal'">
        <UCarousel v-slot="{ item }" v-bind="bind" :items="items" :ui="{ item: 'basis-1/3' }" class="mx-auto max-w-xs w-full">
          <img :src="item.src" class="rounded-lg">
        </UCarousel>
      </template>
    </template>
